...
Const
{$IFDEF WINDOWS}
  External_library='libusb0.dll';
{$ENDIF}
{$IFDEF LINUX}
  External_library='libusb.so';
{$ENDIF}
{$IFDEF DARWIN}
  External_library='libusb.dylib';
{$ENDIF}
...
Type
{$IFDEF WIN32}
   T_usb_open = Function(var dev:TUSBDevice):PUSBDevHandle;Cdecl;
...
Begin
   ...
   {$IFDEF DARWIN}
   if DLLHandle = 0 then
     DLLHandle := LoadLibrary('/opt/local/lib/libusb-1.0.0.dylib'); // .DLL or .so File
   {$ENDIF}
